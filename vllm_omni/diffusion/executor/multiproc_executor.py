...
class MultiprocDiffusionExecutor(DiffusionExecutor):
    ...
    def execute_request(self, scheduler_output: DiffusionSchedulerOutput) -> RunnerOutput:
        ...
        When more than one new request is scheduled in the same step, the
        prompts are merged into a single ``OmniDiffusionRequest`` so the
        worker pipeline can run them through one batched forward pass. The
        merged result is unpacked into a per-request mapping that the
        scheduler / engine then dispatch back to the originating callers.
        """
        from vllm_omni.diffusion.worker.utils import RunnerOutput
        ...
        if not new_reqs:
            raise ValueError(
                "execute_request called with no scheduled new requests."
            )
        ...
        # Multi-request batching: stitch all single-prompt requests into one
        # combined OmniDiffusionRequest. We assume each underlying request
        # has exactly one prompt (the speech-serving entrypoint always does).
        merged_prompts = []
        merged_request_ids = []
        sched_req_ids = []
        for new_req in new_reqs:
            if len(new_req.req.prompts) != 1:
                raise ValueError(
                    "Diffusion request batching expects exactly one prompt "
                    "per scheduled request; got "
                    f"{len(new_req.req.prompts)} for {new_req.sched_req_id}"
                )
            merged_prompts.append(new_req.req.prompts[0])
            req_id = (
                new_req.req.request_ids[0]
                if new_req.req.request_ids
                else new_req.sched_req_id
            )
            merged_request_ids.append(req_id)
            sched_req_ids.append(new_req.sched_req_id)

        head = new_reqs[0].req
        merged_req = OmniDiffusionRequest(
            prompts=merged_prompts,
            sampling_params=head.sampling_params,
            request_ids=merged_request_ids,
        )
        # Best-effort copy of optional fields; absent attributes leave defaults.
        for attr in (
            "negative_prompts",
            "skip_cache_refresh",
            "needs_recv_cache",
            ...
            if hasattr(head, attr) and not hasattr(merged_req, attr):
                continue
            if hasattr(head, attr):
                try:
                    setattr(merged_req, attr, getattr(head, attr))
                except AttributeError:
                    pass

        result = self.collective_rpc(
            "execute_model",
            args=(merged_req, self.od_config),
            unique_reply_rank=0,
            exec_all_ranks=True,
        )
        if not isinstance(result, DiffusionOutput):
        ...
        # Split the batched output into per-request DiffusionOutput entries
        # and stash them on the RunnerOutput so the engine can dispatch each
        # to its waiting caller.
        per_req_results: dict[str, DiffusionOutput] = {}
        outputs = result.output
        if result.error or outputs is None:
            for sched_req_id in sched_req_ids:
                per_req_results[sched_req_id] = DiffusionOutput(
                    error=result.error or "Empty batched output",
                )
        elif isinstance(outputs, list) and len(outputs) == len(sched_req_ids):
            for sched_req_id, audio in zip(sched_req_ids, outputs):
                per_req_results[sched_req_id] = DiffusionOutput(output=audio)
        else:
            raise RuntimeError(
                f"Batched diffusion output has shape {type(outputs).__name__} "
                f"len={len(outputs) if hasattr(outputs, '__len__') else 'N/A'}; "
                f"expected list of {len(sched_req_ids)} entries."
            )

        runner_output = RunnerOutput(
            req_id=sched_req_ids[0],
            step_index=None,
            finished=True,
            result=result,
            per_request_results=per_req_results,
        )
