...
class StageDiffusionProc:
    ...
    def initialize(self) -> None:
        ...
        # When VLLM_OMNI_DIFFUSION_CONCURRENT=1 the engine itself supports
        # multiple in-flight requests via a driver thread. We size the
        # serving-side thread pool to match VLLM_OMNI_DIFFUSION_BATCH_SIZE
        # so HTTP coroutines can actually call engine.step() concurrently;
        # otherwise this pool serializes them at max_workers=1.
        ...
        logger.info(
            "StageDiffusionProc initialized: model=%s, executor max_workers=%d",
            self._model, max_workers,
        )
