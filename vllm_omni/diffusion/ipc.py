...
def _pack_value_if_large(val: object) -> object:
    ...
    if isinstance(val, dict):
        return {k: _pack_value_if_large(v) for k, v in val.items()}
    if isinstance(val, list):
        return [_pack_value_if_large(v) for v in val]
    if isinstance(val, tuple):
        return tuple(_pack_value_if_large(v) for v in val)
    return val
...
def _unpack_if_shm_handle(val: object) -> object:
    ...
    if isinstance(val, dict):
        return {k: _unpack_if_shm_handle(v) for k, v in val.items()}
    if isinstance(val, list):
        return [_unpack_if_shm_handle(v) for v in val]
    if isinstance(val, tuple):
        return tuple(_unpack_if_shm_handle(v) for v in val)
    return val
...
def pack_diffusion_output_shm(output: object) -> object:
    ...
    per_request_results = getattr(output, "per_request_results", None)
    if isinstance(per_request_results, dict):
        output.per_request_results = {
            req_id: _pack_diffusion_fields(result)
            for req_id, result in per_request_results.items()
        }
    return output
...
def unpack_diffusion_output_shm(output: object) -> object:
    ...
    if isinstance(per_request_results, dict):
        output.per_request_results = {
            req_id: _unpack_diffusion_fields(result)
            for req_id, result in per_request_results.items()
        }
    return output
