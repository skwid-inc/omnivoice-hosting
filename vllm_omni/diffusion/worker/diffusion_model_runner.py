...
class DiffusionModelRunner(OmniConnectorModelRunnerMixin):
    ...
    def _update_states_batch(
        self, scheduler_output: DiffusionSchedulerOutput,
    ) -> list[tuple[DiffusionRequestState, bool]]:
        """Step-before update: cleanup finished requests and get/create running states."""
        for req_id in scheduler_output.finished_req_ids:
            self.state_cache.pop(req_id, None)

        states: list[tuple[DiffusionRequestState, bool]] = []
        new_req_by_id = {
            new_req_data.sched_req_id: new_req_data
            for new_req_data in scheduler_output.scheduled_new_reqs
        }
        for req_id in scheduler_output.scheduled_req_ids:
            new_req_data = new_req_by_id.get(req_id)
            if new_req_data is None:
    ...
    def _update_states(self, scheduler_output: DiffusionSchedulerOutput) -> tuple[DiffusionRequestState, bool]:
        ...
        states = self._update_states_batch(scheduler_output)
        if len(states) != 1:
            raise ValueError(
                "This pipeline step path supports batch_size=1, "
                f"but got {len(states)} scheduled requests."
            )
        return states[0]
    ...
    def execute_stepwise(self, scheduler_output: DiffusionSchedulerOutput) -> RunnerOutput:
        ...
        grad_context = torch.no_grad() if use_hsdp else torch.inference_mode()
        with grad_context:
            state_items = self._update_states_batch(scheduler_output)
            ...
            with set_forward_context(vllm_config=self.vllm_config, omni_diffusion_config=self.od_config):
                if hasattr(self.pipeline, "execute_step_batch"):
                    runner_output = self.pipeline.execute_step_batch(state_items)
                    per_request_finished = runner_output.per_request_finished or {}
                    for state, _ in state_items:
                        finished = per_request_finished.get(state.req_id, runner_output.finished)
                        self._update_states_after(state, finished)
                    return runner_output

                if len(state_items) != 1:
                    raise ValueError(
                        "Step mode batch scheduling requires a pipeline with execute_step_batch(); "
                        f"got {len(state_items)} scheduled requests."
                    )

                state, is_new_request = state_items[0]
