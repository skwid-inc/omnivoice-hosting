...
class InlineStageDiffusionClient:
    ...
    def __init__(
        ...
        # When VLLM_OMNI_DIFFUSION_CONCURRENT=1 the engine itself runs a
        # driver thread that batches in-flight requests; we size this pool
        # to match VLLM_OMNI_DIFFUSION_BATCH_SIZE so HTTP coroutines can
        # actually call engine.step() in parallel. Otherwise the pool's
        # max_workers=1 serializes them before they even reach the engine.
        import os
        if os.environ.get("VLLM_OMNI_DIFFUSION_CONCURRENT") == "1":
            try:
                pool_size = max(1, int(os.environ.get(
                    "VLLM_OMNI_DIFFUSION_BATCH_SIZE", "1",
                )))
            except ValueError:
                pool_size = 1
        else:
        ...
        self._executor = ThreadPoolExecutor(
            max_workers=pool_size, thread_name_prefix="inline-diffusion",
        )
        logger.info(
            "[InlineStageDiffusionClient] executor pool size = %d", pool_size,
        )
    ...
    async def _dispatch_request(
        ...
        try:
            ...
            stream_blocks = (
                ...
                and bool(prompt.get("_stream_audio_blocks"))
            )
            if stream_blocks:
                def run_stream() -> None:
                    for result in self._engine.stream(request):
