...
class OmniOpenAIServingSpeech(OpenAIServing, AudioMixin):
    ...
    def _get_omnivoice_voice_names(self) -> set[str]:
        """Return configured OmniVoice preset names without loading audio."""
        cached = getattr(self, "_omnivoice_voice_names", None)
        ...
        if raw_spec:
            ...
            if expanded.exists():
                ...
                try:
                    config = json.loads(raw_spec)
                except json.JSONDecodeError as exc:
                    raise ValueError(
                        "VLLM_OMNI_OMNIVOICE_VOICE_MAP must be a JSON file path "
            ...
            if isinstance(config, dict):
                names = {str(name).lower() for name in config}
        ...
        self._omnivoice_voice_names = names
        return names

    def _resolve_omnivoice_voice_name(self, voice: str | None) -> str | None:
        voice_names = self._get_omnivoice_voice_names()
        voice_name = (voice or "").strip()
        if not voice_names:
            return voice_name or None

        default_voice = os.environ.get("VLLM_OMNI_OMNIVOICE_DEFAULT_VOICE", "").strip()
        if not voice_name:
            voice_name = default_voice or ("default" if "default" in voice_names else "")
        elif (
            voice_name.lower() == "default"
            and "default" not in voice_names
            and default_voice
        ):
        ...
        if not voice_name:
            return None
        if voice_name.lower() not in voice_names:
            available = ", ".join(sorted(voice_names))
            raise ValueError(
    ...
    async def _generate_audio_chunks(
        ...
        try:
            ...
            if raw_request is not None:
                ...
                if engine is None:
                    engine = getattr(self, "_diffusion_engine", None)
    ...
    async def _create_diffusion_speech(
        ...
        try:
            ...
            if request.ref_audio:
                ...
                voice_name = self._resolve_omnivoice_voice_name(request.voice)
                if voice_name:
                    prompt["voice"] = voice_name
            if request.ref_text:
                prompt["ref_text"] = request.ref_text
            if request.language:
                prompt["lang"] = request.language
            ...
            sampling_params_list = [
                p.clone() if hasattr(p, "clone") else copy.deepcopy(p)
                for p in self._diffusion_engine.default_sampling_params_list
            ]
            num_step = int(os.environ.get("VLLM_OMNI_OMNIVOICE_NUM_STEP", "32"))
            for params in sampling_params_list:
                if getattr(params, "num_inference_steps", None) is None:
                    params.num_inference_steps = num_step
                params.step_index = 0
            ...
            if request.stream:
                ...
                if response_format not in ["pcm", "wav"]:
                    return self._diffusion_error_response(
                        "Streaming is only supported for 'pcm' and 'wav' formats. "
                ...
                prompt["_stream_audio_blocks"] = True
    ...
    async def create_speech(
        ...
        """
        if self._diffusion_mode:
            return await self._create_diffusion_speech(request, raw_request=raw_request)
