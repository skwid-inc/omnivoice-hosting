...
# Bucket schedule used when VLLM_OMNI_OMNIVOICE_OPT=1. Mirrors the
# `full_sequence_buckets` list from the standalone OmniVoice benchmark.
...
def _full_sequence_bucket(required_len: int) -> int:
    """Round up to the next size in the fixed bucket schedule.

    When VLLM_OMNI_OMNIVOICE_OPT is unset, returns required_len unchanged so
    default behavior is preserved. Otherwise picks the smallest bucket >=
    required_len, or ceil(required_len / 128) * 128 for very long inputs.
    """
...
class OmniVoiceModel(
    ...
    def _forward_generator(self, input_ids: torch.Tensor, kwargs: dict) -> OmniOutput:
        ...
        # Pad to same length. When VLLM_OMNI_OMNIVOICE_OPT=1 the length is
        # bucketed to a fixed set of sizes so torch.compile graph caches hit
        # across requests (mirrors `_patch_bucketed_generate_iterative` from
        # the standalone OmniVoice benchmark).
