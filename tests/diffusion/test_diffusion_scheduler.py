...
def _make_text_request(req_id: str, text: str) -> OmniDiffusionRequest:
    return OmniDiffusionRequest(
        prompts=[text],
        sampling_params=OmniDiffusionSamplingParams(num_inference_steps=1),
        request_ids=[req_id],
...
def _make_request_output(req_id: str, *, error: str | None = None, finished: bool = True):
    return SimpleNamespace(
        ...
        per_request_results=None,
        per_request_finished=None,
        per_request_step_indices=None,
    )
...
class TestRequestScheduler:
    ...
    def test_batch_size_env_schedules_multiple(self, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setenv("VLLM_OMNI_DIFFUSION_BATCH_SIZE", "3")
        ...
        assert _new_ids(sched_output) == [req_id_a, req_id_b, req_id_c]
        assert sched_output.num_running_reqs == 3
        assert sched_output.num_waiting_reqs == 0

    def test_duration_bucket_strategy_groups_similar_lengths(
        self,
        monkeypatch: pytest.MonkeyPatch,
    ) -> None:
        monkeypatch.setenv("VLLM_OMNI_DIFFUSION_BATCH_SIZE", "3")
        monkeypatch.setenv("VLLM_OMNI_DIFFUSION_BATCH_STRATEGY", "duration_bucket")
        monkeypatch.setenv("VLLM_OMNI_DIFFUSION_DURATION_BUCKET_TOKENS", "64")
        scheduler = RequestScheduler()
        scheduler.initialize(SimpleNamespace())

        req_id_short_a = scheduler.add_request(_make_text_request("short-a", "hello account."))
        req_id_long = scheduler.add_request(
            _make_text_request("long", " ".join(["account"] * 80) + "."),
        )
        req_id_short_b = scheduler.add_request(_make_text_request("short-b", "payment update."))

        sched_output = scheduler.schedule()

        assert _new_ids(sched_output) == [req_id_short_a, req_id_short_b]
        assert sched_output.num_running_reqs == 2
        assert sched_output.num_waiting_reqs == 1
        assert scheduler.get_request_state(req_id_long).status == DiffusionRequestStatus.WAITING
...
class TestStepScheduler:
    ...
    def test_duration_bucket_does_not_fill_running_batch_with_incompatible_waiting(
        self,
        monkeypatch: pytest.MonkeyPatch,
    ) -> None:
        monkeypatch.setenv("VLLM_OMNI_DIFFUSION_BATCH_SIZE", "4")
        monkeypatch.setenv("VLLM_OMNI_DIFFUSION_BATCH_STRATEGY", "duration_bucket")
        monkeypatch.setenv("VLLM_OMNI_DIFFUSION_DURATION_BUCKET_TOKENS", "64")
        scheduler = StepScheduler()
        scheduler.initialize(SimpleNamespace())
        scheduler._duration_estimator = None

        req_id_short_a = scheduler.add_request(
            _make_step_text_request("short-a", "short prompt"),
        )

        first = scheduler.schedule()
        assert _new_ids(first) == [req_id_short_a]
        assert scheduler.update_from_output(
            first,
            _make_step_output(req_id_short_a, step_index=1),
        ) == set()

        req_id_long = scheduler.add_request(
            _make_step_text_request("long", " ".join(["long"] * 80)),
        )
        req_id_short_b = scheduler.add_request(
            _make_step_text_request("short-b", "another short prompt"),
        )

        second = scheduler.schedule()

        assert _new_ids(second) == [req_id_short_b]
        assert _cached_ids(second) == [req_id_short_a]
        assert scheduler.get_request_state(req_id_long).status == DiffusionRequestStatus.WAITING
